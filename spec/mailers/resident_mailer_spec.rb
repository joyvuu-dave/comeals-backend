...
RSpec.describe ResidentMailer, type: :mailer do
  ...
  describe '#password_reset_email' do
    ...
    let(:mail) { ResidentMailer.password_reset_email(resident) }
  ...
  describe '#rotation_signup_email' do
    let(:rotation) { FactoryBot.create(:rotation, community: community) }
    let(:open_dates) { [Date.new(2026, 4, 1), Date.new(2026, 4, 3)] }
    let(:mail) { ResidentMailer.rotation_signup_email(resident, rotation, open_dates, community) }
  ...
  describe '#new_rotation_email' do
    let(:rotation) { FactoryBot.create(:rotation, community: community) }
    let(:mail) { ResidentMailer.new_rotation_email(resident, rotation, community) }
