...
RSpec.describe "Meals API", type: :request do
  ...
  describe "GET /api/v1/meals" do
    ...
    it "filters by date range when start and end are provided" do
      FactoryBot.create(:meal, community: community, date: Date.new(2025, 1, 1))
      new_meal = FactoryBot.create(:meal, community: community, date: Date.new(2025, 6, 1))
  ...
  describe "GET /api/v1/meals/next" do
    it "returns the next upcoming meal" do
      future_meal = FactoryBot.create(:meal, community: community, date: Date.today + 1)

      get "/api/v1/meals/next", params: { token: token }

      expect(response).to have_http_status(:ok)
      expect(JSON.parse(response.body)["meal_id"]).to eq(future_meal.id)
    end
    ...
    it "returns 400 when no future meals exist" do
      ...
      expect(JSON.parse(response.body)["meal_id"]).to be_nil
    end
  end
  ...
  describe "GET /api/v1/meals/:meal_id" do
    ...
    it "returns 403 for a meal in another community" do
      other_community = FactoryBot.create(:community)
      other_meal = FactoryBot.create(:meal, community: other_community)
  ...
  describe "POST /api/v1/meals/:meal_id/residents/:resident_id" do
    ...
    it "sets late and vegetarian flags" do
      ...
      mr = meal.meal_residents.find_by(resident: resident)
      expect(mr.late).to eq(true)
      expect(mr.vegetarian).to eq(true)
    end
    ...
    it "is idempotent — re-signing up updates instead of erroring" do
      ...
      expect(meal.meal_residents.find_by(resident: resident).late).to eq(true)
    end
    ...
    it "rejects signup when meal is at max capacity" do
      other = FactoryBot.create(:resident, community: community, unit: unit, multiplier: 2)
      FactoryBot.create(:meal_resident, meal: meal, resident: other, community: community)
  ...
  describe "PATCH /api/v1/meals/:meal_id/residents/:resident_id" do
    let(:meal) { FactoryBot.create(:meal, community: community) }
    let!(:meal_resident) { FactoryBot.create(:meal_resident, meal: meal, resident: resident, community: community) }
    ...
    it "updates late and vegetarian flags" do
      ...
      expect(meal_resident.late).to eq(true)
      expect(meal_resident.vegetarian).to eq(true)
    end
  end
  ...
  describe "POST /api/v1/meals/:meal_id/residents/:resident_id/guests" do
    ...
    it "sets the vegetarian flag on the guest" do
      ...
      expect(meal.guests.first.vegetarian).to eq(true)
    end
  ...
  describe "PATCH /api/v1/meals/:meal_id/closed" do
    ...
    it "reopens a meal and clears max" do
      ...
      expect(response).to have_http_status(:ok)
      meal.reload
      expect(meal.closed).to eq(false)
