...
RSpec.describe Bill do
  ...
  describe '#unit_cost' do
    it 'divides amount by meal multiplier using BigDecimal' do
      ...
      # multiplier = 2 + 1 = 3
      bill = create(:bill, meal: meal, resident: resident_a, community: community, amount: BigDecimal('50'))
      meal.reload
  ...
  describe '#capped_amount' do
    it 'returns full amount when community has no cap' do
      ...
      bill = create(:bill, meal: meal, resident: resident, community: community, amount: BigDecimal('75'))
      meal.reload
    ...
    it 'proportionally reduces amount when meal cost exceeds cap' do
      ...
      resident_1 = create(:resident, community: capped_community, unit: capped_unit, multiplier: 2)
      resident_2 = create(:resident, community: capped_community, unit: capped_unit, multiplier: 2)

      create(:meal_resident, meal: meal, resident: resident_1, community: capped_community)
      meal.reload

      # multiplier = 2, cap = 2.50, max_cost = 5.00
      # Cook 1 submits $2.00, Cook 2 submits $6.00 = total $8.00 > $5.00 cap
      create(:bill, meal: meal, resident: resident_1, community: capped_community, amount: BigDecimal('2'))
      create(:bill, meal: meal, resident: resident_2, community: capped_community, amount: BigDecimal('6'))
      meal.reload
    ...
    it 'returns full amount when meal cost is under cap' do
      capped_community = create(:community, cap: BigDecimal('25.00'))
      capped_unit = create(:unit, community: capped_community)
      meal = create(:meal, community: capped_community)
      ...
      bill = create(:bill, meal: meal, resident: resident, community: capped_community,
