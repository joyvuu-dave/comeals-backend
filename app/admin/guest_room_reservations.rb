...
ActiveAdmin.register GuestRoomReservation do
  ...
  form do |f|
    f.inputs do
      f.input :resident_id, as: :select, include_blank: false, label: 'Host', collection: Resident.where(community_id: current_admin_user.community_id).includes(:unit).adult.order('units.name ASC').map { |r|
