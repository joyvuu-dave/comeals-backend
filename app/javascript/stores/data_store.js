...
export const DataStore = types
  ...
  .views(self => ({
    ...
    get mealResidentsCount() {
      return Array.from(self.residents.values()).filter(resident => resident.attending)
        .length;
    },
    get attendeesCount() {
    ...
    get vegetarianCount() {
      const vegResidents = Array.from(self.residents
        .values())
        .filter(resident => resident.attending && resident.vegetarian).length;

      const vegGuests = Array.from(self.guests.values()).filter(guest => guest.vegetarian)
        .length;

      return vegResidents + vegGuests;
    },
    get lateCount() {
      return Array.from(self.residents.values()).filter(resident => resident.late).length;
    },
    get extras() {
  ...
  .actions(self => ({
    ...
    submitBills() {
      ...
      if (Array.from(self.bills.values()).some(bill => bill.amountIsValid === false)) {
        self.editBillsMode = true;
        return;
      }

      // Format Bills
      let bills = Array.from(self.bills
        .values())
